...
parser = argparse.ArgumentParser(description='Produce metrics (eff, fakes, etc.) plots on several samples')
...
for tag in RELEASE_TAG :
  ...
  for i_label,i_sample in zip(SAMPLE_LABEL,SAMPLES):
    ...
    for iter_feat in ITER_FEATURES :
      command_plot = "python3 plotting_metrics.py --filesin %s --folderout %s --sample %s --features %s --iter %s"%(listFilein, PATHFILESOUT, i_label, EN_FEATURES, iter_feat)
      if VERBOSE or TEST:
